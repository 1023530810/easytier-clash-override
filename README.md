...
### Clash Verge / FlClash（Mac）

Mac 已原生运行 EasyTier，**不需要 WireGuard 代理**，也不需要覆写脚本。只需在 Clash 的 TUN 设置中排除 EasyTier 网段，避免 Clash TUN 劫持虚拟网络流量。
...
> **为什么不能用 DIRECT 规则？** Clash TUN 的 `auto-route` 会在系统路由表中优先劫持所有流量。即使规则写了 DIRECT，流量也是从 Clash 的 TUN 出去走物理网卡，而不是走 EasyTier 的 TUN 接口。只有通过 `route-exclude-address` 排除网段，才能让 EasyTier TUN 直接处理，延迟最低（~2ms）。
> 
> 详见 `flclash/easytier-override-mac.js` 中的说明。
### 独立配置（无订阅）
